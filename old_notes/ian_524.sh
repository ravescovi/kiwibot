...
##I moved my python install to the conda one here.
## problem seem to be using sudo before pip!!

echo "PATH=" $PATH
pip -V
which pip
pip install --upgrade cryptography
echo "done cryptography"
...
if [ $ROS_NAME == "kinetic" ]; then
	...
	if [ $(dpkg-query -W -f='${Status}' ros-kinetic-desktop-full 2>/dev/null | grep -c "ok installed") -eq 0 
		...
		echo "Installing ROS..."
		sudo sh -c 'echo "deb http://packages.ros.org/ros/ubuntu xenial main" > /etc/apt/sources.list.d/r
os1-latest.list'
		sudo -E apt-key adv --keyserver 'hkp://keyserver.ubuntu.com:80' --recv-key C1CF6E31E6BADE8868B172
B4F42ED6FBAB17C654
		sudo apt-get update
		...
		sudo apt -y install python-rosdep python-rosinstall python-rosinstall-generator python-wstool bui
ld-essential
	...
	if [ $(dpkg-query -W -f='${Status}' ros-melodic-desktop-full 2>/dev/null | grep -c "ok installed") -eq 0 
		...
		echo "Installing ROS..."
		sudo sh -c 'echo "deb http://packages.ros.org/ros/ubuntu bionic main" > /etc/apt/sources.list.d/r
os1-latest.list'
		sudo apt-key adv --keyserver 'hkp://keyserver.ubuntu.com:80' --recv-key C1CF6E31E6BADE8868B172B4F
42ED6FBAB17C654
		sudo apt-get update
...
if [ $INSTALL_TYPE == "full" ]; then
	...
	# STEP 4A: Install librealsense
        echo "--- STEP 4A"
	if [ $(dpkg-query -W -f='${Status}' librealsense2 2>/dev/null | grep -c "ok installed") -eq 0 ]; then
    sudo apt-key adv --keyserver keys.gnupg.net --recv-key F6E65AC044F831AC80A06380C8B3A55A6F3EFCDE || sudo apt-k
ey adv --keyserver hkp://keyserver.ubuntu.com:80 --recv-key F6E65AC044F831AC80A06380C8B3A55A6F3EFCDE
                echo "+++ librealsense 1"
		sudo add-apt-repository "deb http://realsense-hw-public.s3.amazonaws.com/Debian/apt-repo xenial m
ain" -u
                echo "+++ librealsense 2"
		sudo apt-get update
                echo "+++ librealsense 3"
	...
	if [ ! -d "$CAMERA_FOLDER/src" ]; then
		...
		cd $CAMERA_FOLDER/src/
                echo "--- initing catkin workspace"
		catkin_init_workspace
                echo "--- catkin workspace inited"
	fi
	if [ ! -d "$CAMERA_FOLDER/src/realsense-ros" ]; then
		cd $CAMERA_FOLDER/src/
    		git clone https://github.com/IntelRealSense/realsense-ros.git
		cd realsense-ros/
                echo "--- checkout 2.2.13"
		git checkout 2.2.13
                echo "--- checkout 2.2.13 DONE"
	fi
...
if [ $PYTHON_VERSION == "3" ]; then
	...
	cd $LOCOBOT_FOLDER/src/pyrobot/robots/LoCoBot
	source ~/${virtualenv_name}/bin/activate
        echo " --- pip install next"
	pip install --ignore-installed -r requirements_python3.txt
